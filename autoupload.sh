...
DOWNLOAD_PATH='/dev/shm/Download'
...
CLEAN_UP() {
    ...
    [[ -n ${MIN_SIZE} ]] && rcgd --config /etc/ct/.rclone.conf delete -v "${UPLOAD_PATH}" --max-size ${MIN_SIZE}
    [[ -n ${INCLUDE_FILE} ]] && rcgd --config /etc/ct/.rclone.conf delete -v "${UPLOAD_PATH}" --exclude "*.{${INCLUDE_FILE}}"
    [[ -n ${EXCLUDE_FILE} ]] && rcgd --config /etc/ct/.rclone.conf delete -v "${UPLOAD_PATH}" --include "*.{${EXCLUDE_FILE}}"
}
...
UPLOAD_FILE() {
    ...
    while [ ${RETRY} -le ${RETRY_NUM} ]; do
        ...
        rcgd --config /etc/ct/.rclone.conf move -v "${UPLOAD_PATH}" "${REMOTE_PATH}"
        RCLONE_EXIT_CODE=$?
        if [ ${RCLONE_EXIT_CODE} -eq 0 ]; then
            [ -e "${DOT_ARIA2_FILE}" ] && rm -vf "${DOT_ARIA2_FILE}"
            rcgd --config /etc/ct/.rclone.conf rmdirs -v "${DOWNLOAD_PATH}" --leave-root
